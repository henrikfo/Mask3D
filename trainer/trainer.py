...
class InstanceSegmentation(pl.LightningModule):
    ...
    def save_visualizations(
        ...
        def save_ply(name, file_name, xyz, rgb, alpha):
            ...
            cloud.to_file(f"{self.config['general']['save_dir']}/visualizations/{file_name}/{name}.ply")
        ...
        # print(np.asarray(gt_pcd_pos).shape)
        # print(np.asarray(gt_pcd_color).shape)
        # print(np.asarray(gt_pcd_normals).shape)

        # print(np.asarray(pred_coords).shape)
        # print(np.asarray(pred_sem_color).shape)
        # print(np.asarray(pred_inst_color).shape)
        # print(np.asarray(pred_normals).shape)

        v.save(
            f"{self.config['general']['save_dir']}/visualizations/{file_name}", verbose=False
        )
        
        save_ply('RGB', file_name, full_res_coords, original_colors, 0.8)
        
        if backbone_features is not None:
            save_ply('PCA', file_name, full_res_coords, backbone_features, 0.8)

        if "labels" in target_full:
            save_ply('Semantics', file_name, gt_pcd_pos, gt_pcd_color, 0.8)
            save_ply('Instances', file_name, gt_pcd_pos, gt_inst_pcd_color, 0.8)

        if len(pred_coords) > 0:
            save_ply('Pred Semantics', file_name, pred_coords, pred_sem_color, 0.8)
            save_ply('Pred Instances', file_name, pred_coords, pred_inst_color, 0.8)
    ...
    def eval_instance_epoch_end(self):
        ...
        if not os.path.exists(f"results/{self.config.general.experiment_name}"):  
            os.makedirs(f"results/{self.config.general.experiment_name}")
            
        pred_path_own = f"results/{self.config.general.experiment_name}/{self.current_epoch}.txt"
        ...
        if self.validation_dataset.dataset_name == "s3dis":
            ...
            mprec, mrec = evaluate(
                new_preds, gt_data_path, pred_path_own, dataset="s3dis"
            )
            ...
            evaluate(new_preds, gt_data_path, pred_path_own, dataset="stpls3d")
        else:
            evaluate(
                self.preds,
                gt_data_path,
        ...
        with open(pred_path_own, "r") as fin:
